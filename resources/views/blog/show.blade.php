        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white rounded-xl shadow-xl overflow-hidden">
                @if ($post->featured_image)
                    <img src="{{ asset('storage/' . $post->featured_image) }}" alt="{{ $post->title }} featured image"
                ...
                <div class="p-6 md:p-10">
                    <div class="mb-6">
                        ...
                        @if ($post->category)
                            ...
                            @endphp
                            @if($categoryName)
                            <a href="{{ route('blog.index', ['category' => $categoryName]) }}" class="inline-block bg-sky-100 text-sky-700 px-3 py-1 rounded-full text-sm font-semibold hover:bg-sky-200 transition mb-2">
                                {{ $categoryName }}
                            </a>
                            @endif
                        @endif
                        <h1 class="text-3xl md:text-4xl lg:text-5xl font-extrabold text-slate-900 tracking-tight">{{ $post->title }}</h1>
                        <p class="mt-3 text-base text-slate-500">
                            Published on <time datetime="{{ $post->published_at->toDateString() }}">{{ $post->published_at->format('F j, Y') }}</time>
                        </p>
                    </div>
                        ...
                        <div class="mt-8 pt-6 border-t border-slate-200">
                            ...
                            <div class="flex flex-wrap gap-2">
                                @foreach ($post->tags as $tag)
                                    ...
                                    @endphp
                                    @if($tagName)
                                    <a href="{{ route('blog.index', ['tag' => $tagName]) }}" class="inline-block bg-slate-100 text-slate-600 px-3 py-1 rounded-full text-xs font-medium hover:bg-slate-200 hover:text-slate-700 transition">
                                        #{{ $tagName }}
                                    </a>
                                    @endif
                                @endforeach
